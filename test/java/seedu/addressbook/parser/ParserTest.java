...
public class ParserTest {
    ...
    public void parse_updateCommandInvalidArgs_errorMessage() {
        final String[] inputs = {
                ...
                // no email prefix
                String.format("update 1 %s p/%s %s a/%s", Name.EXAMPLE, Phone.EXAMPLE, Email.EXAMPLE, Address.EXAMPLE),
                // no address prefix
                String.format("update 1 %s p/%s e/%s %s", Name.EXAMPLE, Phone.EXAMPLE, Email.EXAMPLE, Address.EXAMPLE)
        };
    ...
    public void parse_updateCommandInvalidPersonDataInArgs_errorMessge() {
        ...
        // address can be any string, so no invalid address
        final String updateCommandFormatString = "update 1 %s %s %s a/" + Address.EXAMPLE;
